...
class Nav extends HTMLElement {
  ...
  get template() {
    return html`
      <nav
        class="md:fixed w-52 print:hidden p-3 -mx-3"
        aria-label="Menu"
        tabindex="0"
      >
        <ul class="flex flex-col gap-3">
              ...
              html`
                <li>
                  <a
                    role="button"
                    class="link"
                    aria-label=${`${getContent("nav.goTo")} ${section}`}
                    href=${`#${section}`}
                  >
          ...
          <li>
            <button
              class="visible-on-focus link focus:link"
              @click=${this.toggleReduceMotion}
            >
              ${this.userPreferMotionReduce
                ? getContent("nav.motionReduced")
                : getContent("nav.motionFull")}
            </button>
            <span class="sr-only" aria-live="off" id="aria-live-feedback">
              ${this.userPreferMotionReduce
                ? getContent("nav.motionReducedFeedback")
                : getContent("nav.motionFullFeedback")}
            </span>
          </li>
        </ul>
      </nav>
